 ...
 *  @since Thu Apr 12 2018 00:14:07 GMT+0300 (MSK)
 *  @version 5.1.0 (RU)
 *  @tutorial https://multicaptchabot.wixsite.com/multicaptchabot/instruction
 *  @author AlexanderFSP <https://github.com/AlexanderFSP>
 *
 *  [ Инструкция ]:
 * 	⑴ Скачайте Mozilla Firefox, iMacros v8.9.7 и настройте их согласно README.md (https://github.com/AlexanderFSP/MultiCaptchaBot/blob/master/README.md) или инструкции на официальном сайте (https://multicaptchabot.wixsite.com/multicaptchabot/instruction)
 *   	⑵ Обязательно авторизируйтесь на всех сайтах, на которых Вы будете использовать бота
 *   	⑶ Зарегистрируйтесь и пополните счёт на сервисе распознавания капчи
 *   	⑷ Внимание, не забудьте проделать следующие действия:
 *      	▻ Создать папку C:\MultiCaptcha_bot
 *       	▻ Вставить Ваш API KEY с сервиса ruCaptcha.com / 2Captcha.com (Строка №29)
 *       	▻ Активировать нужные краны и дополнительные функции на них (Строки №31 - 44)
 * 		▻ Если Вы собираетесь использовать кран https://freenem.com/, то необходимо установить любое расширение для блокировки рекламы. Советую установить Adblock Plus
 *		Ссылка на скачивание: https://adblockplus.org/en/
 * 
 *  [ Связь со мной ]: 
...
const captchaPath      = 'C:\\MultiCaptcha_bot\\';         // Путь для загрузки текстовых капч (Не забудьте про двойные слеши)
const logPath 	       = 'C:\\MultiCaptcha_bot\\log.txt';  // Путь для файла с логом (Не забудьте про двойные слеши)

const apiKey_ruCaptcha = '******************************'; // Ваш API KEY с сервиса ruCaptcha.com / 2Captcha.com

// [ Блок №2 ]: Кран https://freebitco.in/
const freeBITCOIN              = 'OFF';  // [ON / OFF] - Активировать бота на проекте https://freebitco.in/
const freeBITCOIN_RewardPoints = 'OFF';  // [ON / OFF] - Авто-использование Reward Points на сайте https://freebitco.in/ (Полный алгоритм можно найти здесь: https://github.com/AlexanderFSP/MultiCaptchaBot)
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
/**
 *  @description Решение reCAPTCHA v2 через сервис https://ruCaptcha.com (http://discount.ruCaptcha.com) / http://2Captcha.com (http://discount.2Captcha.com)
 ...
 *  @returns { JSON } Объект, который содержит информацию об успешном / неудачном запросе к серверу - https://ruCaptcha.com (http://discount.ruCaptcha.com) / http://2Captcha.com (http://discount.2Captcha.com)
 */
...
/**
 *  @description Решение текстовой капчи через сервис https://ruCaptcha.com / http://2Captcha.com
 ...
 *  @returns { JSON } Объект, который содержит информацию об успешном / неудачном запросе к сервису серверу - https://ruCaptcha.com / http://2Captcha.com
 */
 ...
 *  @param { String } pageurl RandomTimer Опция 'Случайный таймер'
	...
	try {
		while (true) {
				...
				freeBitcoinBody : {
					...
					while (solvingCaptchaCycles <= 5) {
						...
						if (Number(window.content.document.getElementById('winnings').innerHTML) > 0) {
							...
							log('freebitco.in', 'Успешный сбор! Собрано: ' + Number(window.content.document.getElementById('winnings').innerHTML).toFixed(8) + ' BTC; ' + Number(window.content.document.getElementById('fp_reward_points_won').innerHTML) + ' Reward Points; ' + Number(window.content.document.getElementById('fp_lottery_tickets_won').innerHTML) + ' Lottery Tickets');
			...
			if (freeDOGECOIN === 'ON') {
					...
					if (Number(window.content.document.getElementById('winnings').innerHTML) > 0) {
						iimDisplay('Successfully claimed!');
						log('freedoge.co.in', 'Успешный сбор! Собрано: ' + Number(window.content.document.getElementById('winnings').innerHTML).toFixed(8) + ' DOGE ');
				...
				freeNemBody : {
					...
					while (authorizationCycles <= 5) {
						...
						if (!window.content.document.getElementsByClassName('main-button-2 roll-button').length) {
							...
							if (!window.content.document.getElementsByClassName('main-button-2 roll-button').length) {
								iimDisplay('reCAPTCHA v2 is detected. Solving...');
								log('freenem.com', 'reCAPTCHA v2 обнаружена. Попытаемся ее решить...');
					...
					while (solvingCaptchaCycles <= 5) {
						...
						if (!isNaN(winningDuringLastSession) || (winningDuringLastSession > 0)) {
							iimDisplay('Claiming NEM...');	log('freenem.com', 'Собираем NEM...');
							iimDisplay('Successfully claimed!');	log('freenem.com', 'Успешный сбор! Собрано: ' + winningDuringLastSession.toFixed(8) + ' NEM');
							...
							if (!isNaN(winningDuringLastSession) || (winningDuringLastSession > 0)) {
								iimDisplay('Successfully claimed!');
								log('freenem.com', 'Успешный сбор! Собрано: ' + winningDuringLastSession.toFixed(8) + ' NEM');
								Winnings_freeNEM += winningDuringLastSession;
								break;
							} else {
								iimDisplay('Captcha was solved incorrectly. Sending a report and trying to solve captcha again...');
